 ...
 * This class makes the TST and inverted index table from the text files.
 */
public class BuildEngine implements Constants {
    ...
    /**
     * This is the flow chart for our build Engine. Here we are making our ternary search tree and inverted index table. ​
     * For each file downloaded using crawler, ​
     * firstly we have read the file content and then the content is tokenized. ​
     * While reading the tokens generated we got the word count and the word set,​
     * From the word set all the stop words are removed. Then each word is inserted in the Ternary search tree, and for each word Inverted index is updated. Before inserting in the ternary search tree, we have checked, if the word is already present in the tree,  then we got its index form the ternary search tree and  only  inverted index is updated. All the above steps are repeated for each file.​
     * At last we have stored our TST and index table in  the files.  so that we need not to make index table and TST each time we run search query. ​
     */
